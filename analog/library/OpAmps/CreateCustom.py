...
from scripts.XSchemVariantOptimizer import optimize_circuit
...
BOUNDS = [
    # Width bounds - minimum 0.42μm for SKY130
    ...
    # Length bounds - minimum 0.15μm for 1.8V SKY130 devices
...
if __name__ == "__main__":
    result = optimize_circuit(
        "OpAmp", INITIAL_PARAMS, TESTS, TARGETS, BOUNDS,
        template_dir="template", max_iterations=25, target_precision=0.95
    )
    
    print("\n=== OP-AMP OPTIMIZATION RESULTS) ===")
    for bound in BOUNDS:
        comp = bound["component"]
        param = bound["parameter"] 
        initial = float(INITIAL_PARAMS[comp][param])
        final = float(result[comp][param])
        change = final - initial
        print(f"  {comp}.{param}: {initial:.3f} → {final:.3f} (Δ{change:+.3f})")
    
    print(f"\n=== FINAL OP-AMP PARAMETERS ===")
    total_area = 0
    for comp in INITIAL_PARAMS:
        if comp in result:
            w_val = float(result[comp]["W"]) if "W" in result[comp] else float(INITIAL_PARAMS[comp]["W"])
            l_val = float(result[comp]["L"]) if "L" in result[comp] else float(INITIAL_PARAMS[comp]["L"])
            area = w_val * l_val
            total_area += area
            print(f"  {comp}: W={w_val:.3f}µm, L={l_val:.3f}µm (Area: {area:.1f}µm²)")
    
    print(f"\nTotal Transistor Area: {total_area:.1f}µm²")
