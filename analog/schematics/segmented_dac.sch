v {xschem version=3.4.7 file_version=1.2}
G {}
K {}
V {}
S {}
E {}
N -1110 -90 -1110 -50 {
lab=#net1}
N -1110 -90 -1070 -90 {
lab=#net1}
N -1070 -120 -1070 -90 {
lab=#net1}
N -1110 10 -1070 10 {
lab=I_main}
N -1070 -20 -1070 10 {
lab=I_main}
N -420 -90 -420 -50 {
lab=#net2}
N -1110 -180 -1110 -150 {
lab=V_in}
N -280 -90 -280 -50 {
lab=#net3}
N -480 -80 -320 -80 {
lab=#net1}
N -320 -120 -320 -80 {
lab=#net1}
N -320 -60 -320 -20 {
lab=I_main}
N -140 -90 -140 -50 {
lab=#net4}
N -180 -120 -180 -80 {
lab=#net1}
N -180 -60 -180 -20 {
lab=I_main}
N 0 -90 0 -50 {
lab=#net5}
N -40 -120 -40 -80 {
lab=#net1}
N -40 -60 -40 -20 {
lab=I_main}
N -320 -80 -180 -80 {
lab=#net1}
N -180 -80 -40 -80 {
lab=#net1}
N -180 -60 -40 -60 {
lab=I_main}
N -320 -60 -180 -60 {
lab=I_main}
N -420 -150 -280 -150 {
lab=V_in}
N -280 -150 -140 -150 {
lab=V_in}
N -140 -150 0 -150 {
lab=V_in}
N -1110 10 -1110 50 {
lab=I_main}
N -420 10 -420 40 {
lab=#net6}
N -450 40 -420 40 {
lab=#net6}
N -280 10 -280 40 {
lab=#net7}
N -310 40 -280 40 {
lab=#net7}
N -140 10 -140 40 {
lab=#net8}
N -170 40 -140 40 {
lab=#net8}
N 0 10 0 40 {
lab=#net9}
N -30 40 0 40 {
lab=#net9}
N -420 120 0 120 {
lab=I_out_n}
N -450 140 -30 140 {
lab=I_out_p}
N -420 100 -420 120 {
lab=I_out_n}
N -280 100 -280 120 {
lab=I_out_n}
N -140 100 -140 120 {
lab=I_out_n}
N 0 100 0 120 {
lab=I_out_n}
N -30 100 -30 140 {
lab=I_out_p}
N -170 100 -170 140 {
lab=I_out_p}
N -310 100 -310 140 {
lab=I_out_p}
N -450 100 -450 140 {
lab=I_out_p}
N -420 10 -420 40 {
lab=#net6}
N -450 40 -420 40 {
lab=#net6}
N 0 120 100 120 {
lab=I_out_n}
N -490 70 -490 160 {
lab=T2}
N -350 70 -350 160 {
lab=T1}
N -210 70 -210 160 {
lab=T0}
N -70 70 -70 160 {
lab=B4}
N -380 70 -380 180 {
lab=T2_n}
N -240 70 -240 180 {
lab=T1_n}
N -100 70 -100 180 {
lab=T0_n}
N 40 70 40 180 {
lab=B4_n}
N 140 -90 140 -50 {
lab=#net10}
N 280 -90 280 -50 {
lab=#net11}
N 100 -120 100 -80 {
lab=#net1}
N 240 -120 240 -80 {
lab=#net1}
N 100 -60 100 -20 {
lab=I_main}
N 240 -60 240 -20 {
lab=I_main}
N 420 -90 420 -50 {
lab=#net12}
N 380 -120 380 -80 {
lab=#net1}
N 380 -60 380 -20 {
lab=I_main}
N 560 -90 560 -50 {
lab=#net13}
N 520 -120 520 -80 {
lab=#net1}
N 520 -60 520 -20 {
lab=I_main}
N 240 -80 380 -80 {
lab=#net1}
N 380 -80 520 -80 {
lab=#net1}
N 380 -60 520 -60 {
lab=I_main}
N 240 -60 380 -60 {
lab=I_main}
N 140 -150 280 -150 {
lab=V_in}
N 280 -150 420 -150 {
lab=V_in}
N 420 -150 560 -150 {
lab=V_in}
N 140 10 140 40 {
lab=#net14}
N 110 40 140 40 {
lab=#net14}
N 280 10 280 40 {
lab=#net15}
N 250 40 280 40 {
lab=#net15}
N 420 10 420 40 {
lab=#net16}
N 390 40 420 40 {
lab=#net16}
N 560 10 560 40 {
lab=#net17}
N 530 40 560 40 {
lab=#net17}
N 140 120 560 120 {
lab=I_out_n}
N 110 140 530 140 {
lab=I_out_p}
N 140 100 140 120 {
lab=I_out_n}
N 280 100 280 120 {
lab=I_out_n}
N 420 100 420 120 {
lab=I_out_n}
N 560 100 560 120 {
lab=I_out_n}
N 530 100 530 140 {
lab=I_out_p}
N 390 100 390 140 {
lab=I_out_p}
N 250 100 250 140 {
lab=I_out_p}
N 110 100 110 140 {
lab=I_out_p}
N 140 10 140 40 {
lab=#net14}
N 110 40 140 40 {
lab=#net14}
N 560 120 660 120 {
lab=I_out_n}
N 530 140 630 140 {
lab=I_out_p}
N 70 70 70 160 {
lab=B3}
N 210 70 210 160 {
lab=B2}
N 350 70 350 160 {
lab=B1}
N 490 70 490 160 {
lab=B0}
N 180 70 180 180 {
lab=B3_n}
N 320 70 320 180 {
lab=B2_n}
N 460 70 460 180 {
lab=B1_n}
N 600 70 600 180 {
lab=B0_n}
N -30 140 110 140 {
lab=I_out_p}
N 100 120 140 120 {
lab=I_out_n}
N -40 -80 100 -80 {
lab=#net1}
N -40 -60 100 -60 {
lab=I_main}
N 100 -80 240 -80 {
lab=#net1}
N 100 -60 240 -60 {
lab=I_main}
N 0 -150 140 -150 {
lab=V_in}
N 390 70 420 70 {lab=#net16}
N 530 70 560 70 {lab=#net17}
N 250 70 280 70 {lab=#net15}
N 110 70 140 70 {lab=#net14}
N -170 70 -140 70 {lab=#net8}
N -310 70 -280 70 {lab=#net7}
N -450 70 -420 70 {lab=#net6}
N -420 -20 -400 -20 {lab=#net2}
N -400 -50 -400 -20 {lab=#net2}
N -420 -50 -400 -50 {lab=#net2}
N -280 -20 -260 -20 {lab=#net3}
N -260 -50 -260 -20 {lab=#net3}
N -280 -50 -260 -50 {lab=#net3}
N -140 -20 -120 -20 {lab=#net4}
N -120 -50 -120 -20 {lab=#net4}
N -140 -50 -120 -50 {lab=#net4}
N 0 -20 20 -20 {lab=#net5}
N 20 -50 20 -20 {lab=#net5}
N 0 -50 20 -50 {lab=#net5}
N 140 -20 160 -20 {lab=#net10}
N 160 -50 160 -20 {lab=#net10}
N 140 -50 160 -50 {lab=#net10}
N 280 -20 300 -20 {lab=#net11}
N 300 -50 300 -20 {lab=#net11}
N 280 -50 300 -50 {lab=#net11}
N 420 -20 440 -20 {lab=#net12}
N 440 -50 440 -20 {lab=#net12}
N 420 -50 440 -50 {lab=#net12}
N 560 -20 580 -20 {lab=#net13}
N 580 -50 580 -20 {lab=#net13}
N 560 -50 580 -50 {lab=#net13}
N 560 -120 580 -120 {lab=V_in}
N 580 -150 580 -120 {lab=V_in}
N 560 -150 580 -150 {lab=V_in}
N 420 -120 440 -120 {lab=V_in}
N 440 -150 440 -120 {lab=V_in}
N 280 -120 300 -120 {lab=V_in}
N 300 -150 300 -120 {lab=V_in}
N 140 -120 160 -120 {lab=V_in}
N 160 -150 160 -120 {lab=V_in}
N 0 -120 20 -120 {lab=V_in}
N 20 -150 20 -120 {lab=V_in}
N -140 -120 -120 -120 {lab=V_in}
N -120 -150 -120 -120 {lab=V_in}
N -280 -120 -260 -120 {lab=V_in}
N -260 -150 -260 -120 {lab=V_in}
N -420 -120 -400 -120 {lab=V_in}
N -400 -150 -400 -120 {lab=V_in}
N -430 40 -430 70 {lab=#net6}
N -290 40 -290 70 {lab=#net7}
N -150 40 -150 70 {lab=#net8}
N -10 40 -10 70 {lab=#net9}
N 130 40 130 70 {lab=#net14}
N 270 40 270 70 {lab=#net15}
N 410 40 410 70 {lab=#net16}
N 550 40 550 70 {lab=#net17}
N -1130 -20 -1110 -20 {lab=#net1}
N -1130 -50 -1130 -20 {lab=#net1}
N -1130 -50 -1110 -50 {lab=#net1}
N -1130 -120 -1110 -120 {lab=V_in}
N -1130 -150 -1130 -120 {lab=V_in}
N -1130 -150 -1110 -150 {lab=V_in}
N -30 70 -0 70 {lab=#net9}
N 660 120 730 120 {lab=I_out_n}
N -460 -60 -320 -60 {lab=I_main}
N -460 -60 -460 -20 {lab=I_main}
N -460 -120 -460 -80 {lab=#net1}
N -840 -90 -840 -50 {
lab=#net18}
N -700 -90 -700 -50 {
lab=#net19}
N -740 -120 -740 -80 {
lab=#net1}
N -740 -60 -740 -20 {
lab=I_main}
N -560 -90 -560 -50 {
lab=#net20}
N -600 -120 -600 -80 {
lab=#net1}
N -600 -60 -600 -20 {
lab=I_main}
N -740 -80 -600 -80 {
lab=#net1}
N -740 -60 -600 -60 {
lab=I_main}
N -840 -150 -700 -150 {
lab=V_in}
N -700 -150 -560 -150 {
lab=V_in}
N -840 10 -840 40 {
lab=#net21}
N -870 40 -840 40 {
lab=#net21}
N -700 10 -700 40 {
lab=#net22}
N -730 40 -700 40 {
lab=#net22}
N -560 10 -560 40 {
lab=#net23}
N -590 40 -560 40 {
lab=#net23}
N -840 100 -840 120 {
lab=I_out_n}
N -700 100 -700 120 {
lab=I_out_n}
N -560 100 -560 120 {
lab=I_out_n}
N -590 100 -590 140 {
lab=I_out_p}
N -730 100 -730 140 {
lab=I_out_p}
N -870 100 -870 140 {
lab=I_out_p}
N -840 10 -840 40 {
lab=#net21}
N -870 40 -840 40 {
lab=#net21}
N -910 70 -910 160 {
lab=T5}
N -770 70 -770 160 {
lab=T4}
N -630 70 -630 160 {
lab=T3}
N -800 70 -800 180 {
lab=T5_n}
N -660 70 -660 180 {
lab=T4_n}
N -520 70 -520 180 {
lab=T3_n}
N -590 70 -560 70 {lab=#net23}
N -730 70 -700 70 {lab=#net22}
N -870 70 -840 70 {lab=#net21}
N -840 -20 -820 -20 {lab=#net18}
N -820 -50 -820 -20 {lab=#net18}
N -840 -50 -820 -50 {lab=#net18}
N -700 -20 -680 -20 {lab=#net19}
N -680 -50 -680 -20 {lab=#net19}
N -700 -50 -680 -50 {lab=#net19}
N -560 -20 -540 -20 {lab=#net20}
N -540 -50 -540 -20 {lab=#net20}
N -560 -50 -540 -50 {lab=#net20}
N -560 -120 -540 -120 {lab=V_in}
N -540 -150 -540 -120 {lab=V_in}
N -700 -120 -680 -120 {lab=V_in}
N -680 -150 -680 -120 {lab=V_in}
N -840 -120 -820 -120 {lab=V_in}
N -820 -150 -820 -120 {lab=V_in}
N -850 40 -850 70 {lab=#net21}
N -710 40 -710 70 {lab=#net22}
N -570 40 -570 70 {lab=#net23}
N -880 -60 -740 -60 {lab=I_main}
N -880 -60 -880 -20 {lab=I_main}
N -880 -120 -880 -80 {lab=#net1}
N -560 -150 -420 -150 {lab=V_in}
N -840 120 -420 120 {lab=I_out_n}
N -870 140 -450 140 {lab=I_out_p}
N -980 -90 -980 -50 {
lab=#net24}
N -980 10 -980 40 {
lab=#net25}
N -1010 40 -980 40 {
lab=#net25}
N -980 100 -980 120 {
lab=I_out_n}
N -1010 100 -1010 140 {
lab=I_out_p}
N -980 10 -980 40 {
lab=#net25}
N -1010 40 -980 40 {
lab=#net25}
N -1050 70 -1050 160 {
lab=T6}
N -940 70 -940 180 {
lab=T6_n}
N -1010 70 -980 70 {lab=#net25}
N -980 -20 -960 -20 {lab=#net24}
N -960 -50 -960 -20 {lab=#net24}
N -980 -50 -960 -50 {lab=#net24}
N -980 -120 -960 -120 {lab=V_in}
N -960 -150 -960 -120 {lab=V_in}
N -990 40 -990 70 {lab=#net25}
N -1020 -60 -1020 -20 {lab=I_main}
N -1020 -120 -1020 -80 {lab=#net1}
N -980 -150 -840 -150 {lab=V_in}
N -980 120 -840 120 {lab=I_out_n}
N -1010 140 -870 140 {lab=I_out_p}
N -600 -80 -480 -80 {lab=#net1}
N -600 -60 -460 -60 {lab=I_main}
N -880 -80 -740 -80 {lab=#net1}
N -1020 -80 -880 -80 {lab=#net1}
N -1020 -60 -880 -60 {lab=I_main}
N -1070 -120 -1020 -120 {lab=#net1}
N -1070 -20 -1020 -20 {lab=I_main}
C {sky130_fd_pr/pfet_01v8.sym} -440 -120 0 0 {name=M1
W=min_width
L=length
nf=1
mult=32
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -440 -20 0 0 {name=M2
W=min_width
L=length
nf=1
mult=32
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -1090 -120 0 1 {name=M3
W=min_width
L=length
nf=1
mult=128
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -1090 -20 0 1 {name=M4
W=min_width
L=length
nf=1
mult=128
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -300 -120 0 0 {name=M5
W=min_width
L=length
nf=1
mult=32
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -300 -20 0 0 {name=M6
W=min_width
L=length
nf=1
mult=32
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -160 -120 0 0 {name=M7
W=min_width
L=length
nf=1
mult=32
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -160 -20 0 0 {name=M8
W=min_width
L=length
nf=1
mult=32
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -20 -120 0 0 {name=M9
W=min_width
L=length
nf=1
mult=16
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -20 -20 0 0 {name=M10
W=min_width
L=length
nf=1
mult=16
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -470 70 0 0 {name=M12
W=min_width*switch_multiplier
L=length
nf=1
mult=32
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -400 70 0 1 {name=M13
W=min_width*switch_multiplier
L=length
nf=1
mult=32
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -330 70 0 0 {name=M14
W=min_width*switch_multiplier
L=length
nf=1
mult=32
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -260 70 0 1 {name=M15
W=min_width*switch_multiplier
L=length
nf=1
mult=32
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -190 70 0 0 {name=M16
W=min_width*switch_multiplier
L=length
nf=1
mult=32
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -120 70 0 1 {name=M17
W=min_width*switch_multiplier
L=length
nf=1
mult=32
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -50 70 0 0 {name=M18
W=min_width*switch_multiplier
L=length
nf=1
mult=16
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} 20 70 0 1 {name=M19
W=min_width*switch_multiplier
L=length
nf=1
mult=16
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {ipin.sym} 490 160 2 0 {name=p3 lab=B0
L=length}
C {ipin.sym} 600 180 0 0 {name=p5 lab=B0_n
L=length}
C {ipin.sym} 350 160 2 0 {name=p6 lab=B1
W=50
L=length}
C {ipin.sym} 210 160 2 0 {name=p7 lab=B2
W=min_width
L=length}
C {ipin.sym} 70 160 2 0 {name=p8 lab=B3
W=min_width
L=length}
C {ipin.sym} 460 180 0 0 {name=p9 lab=B1_n
L=length}
C {ipin.sym} 320 180 0 0 {name=p10 lab=B2_n
W=min_width
L=length}
C {ipin.sym} 180 180 0 0 {name=p11 lab=B3_n
W=min_width
L=length}
C {sky130_fd_pr/pfet_01v8.sym} 120 -120 0 0 {name=M11
W=min_width
L=length
nf=1
mult=8
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} 120 -20 0 0 {name=M20
W=min_width
L=length
nf=1
mult=8
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} 260 -120 0 0 {name=M21
W=min_width
L=length
nf=1
mult=4
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} 260 -20 0 0 {name=M22
W=min_width
L=length
nf=1
mult=4
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} 400 -120 0 0 {name=M23
W=min_width
L=length
nf=1
mult=2
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} 400 -20 0 0 {name=M24
W=min_width
L=length
nf=1
mult=2
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} 540 -120 0 0 {name=M25
W=min_width
L=length
nf=1
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} 540 -20 0 0 {name=M26
W=min_width
L=length
nf=1
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} 90 70 0 0 {name=M27
W=min_width*switch_multiplier
L=length
nf=1
mult=8
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} 160 70 0 1 {name=M28
W=min_width*switch_multiplier
L=length
nf=1
mult=8
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} 230 70 0 0 {name=M29
W=min_width*switch_multiplier
L=length
nf=1
mult=4
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} 300 70 0 1 {name=M30
W=min_width*switch_multiplier
L=length
nf=1
mult=4
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} 370 70 0 0 {name=M31
W=min_width*switch_multiplier
L=length
nf=1
mult=2
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} 440 70 0 1 {name=M32
W=min_width*switch_multiplier
L=length
nf=1
mult=2
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} 510 70 0 0 {name=M33
W=min_width*switch_multiplier
L=length
nf=1
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} 580 70 0 1 {name=M34
W=min_width*switch_multiplier
L=length
nf=1
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {ipin.sym} -70 160 2 0 {name=p12 lab=B4
W=min_width
L=length}
C {ipin.sym} 40 180 0 0 {name=p13 lab=B4_n
W=min_width
L=length}
C {ipin.sym} -210 160 2 0 {name=p14 lab=T0
W=min_width
L=length}
C {ipin.sym} -350 160 2 0 {name=p15 lab=T1
W=min_width
L=length
mult=32}
C {ipin.sym} -490 160 2 0 {name=p16 lab=T2
W=min_width
L=length
mult=32}
C {ipin.sym} -100 180 0 0 {name=p17 lab=T0_n
W=min_width
L=length}
C {ipin.sym} -240 180 0 0 {name=p18 lab=T1_n
W=min_width
L=length
mult=32}
C {ipin.sym} -380 180 0 0 {name=p19 lab=T2_n
W=min_width
L=length
mult=32}
C {devices/opin.sym} 630 140 0 0 {name=p2 lab=I_out_p
L=length}
C {ipin.sym} -180 -260 2 0 {name=p21 lab=V_in}
C {devices/lab_pin.sym} -210 -150 1 0 {name=p22 sig_type=std_logic lab=V_in
W=min_width
L=length}
C {devices/lab_pin.sym} -1110 -180 2 0 {name=p4 sig_type=std_logic lab=V_in
W=min_width
L=length}
C {devices/lab_pin.sym} -180 -260 0 0 {name=p23 sig_type=std_logic lab=V_in}
C {devices/code_shown.sym} 180 -240 0 0 {name=s1 only_toplevel=false value=".param min_width=0.42
.param switch_multiplier=1
.param length=0.425"
W=min_width
L=length}
C {devices/opin.sym} 730 120 0 0 {name=p1 lab=I_out_n
L=length}
C {devices/opin.sym} -1110 50 2 0 {name=p20 lab=I_main
L=length}
C {sky130_fd_pr/pfet_01v8.sym} -860 -120 0 0 {name=M35
W=min_width
L=length
nf=1
mult=32
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -860 -20 0 0 {name=M36
W=min_width
L=length
nf=1
mult=32
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -720 -120 0 0 {name=M37
W=min_width
L=length
nf=1
mult=32
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -720 -20 0 0 {name=M38
W=min_width
L=length
nf=1
mult=32
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -580 -120 0 0 {name=M39
W=min_width
L=length
nf=1
mult=32
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -580 -20 0 0 {name=M40
W=min_width
L=length
nf=1
mult=32
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -890 70 0 0 {name=M41
W=min_width*switch_multiplier
L=length
nf=1
mult=32
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -820 70 0 1 {name=M42
W=min_width*switch_multiplier
L=length
nf=1
mult=32
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -750 70 0 0 {name=M43
W=min_width*switch_multiplier
L=length
nf=1
mult=32
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -680 70 0 1 {name=M44
W=min_width*switch_multiplier
L=length
nf=1
mult=32
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -610 70 0 0 {name=M45
W=min_width*switch_multiplier
L=length
nf=1
mult=32
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -540 70 0 1 {name=M46
W=min_width*switch_multiplier
L=length
nf=1
mult=32
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {ipin.sym} -630 160 2 0 {name=p24 lab=T3
W=min_width
L=length
mult=32}
C {ipin.sym} -770 160 2 0 {name=p25 lab=T4
W=min_width
L=length
mult=32}
C {ipin.sym} -910 160 2 0 {name=p26 lab=T5
W=min_width
L=length
mult=32}
C {ipin.sym} -520 180 0 0 {name=p27 lab=T3_n
W=min_width
L=length
mult=32}
C {ipin.sym} -660 180 0 0 {name=p28 lab=T4_n
W=min_width
L=length
mult=32}
C {ipin.sym} -800 180 0 0 {name=p29 lab=T5_n
W=min_width
L=length
mult=32}
C {sky130_fd_pr/pfet_01v8.sym} -1000 -120 0 0 {name=M47
W=min_width
L=length
nf=1
mult=32
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -1000 -20 0 0 {name=M48
W=min_width
L=length
nf=1
mult=32
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -1030 70 0 0 {name=M49
W=min_width*switch_multiplier
L=length
nf=1
mult=32
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -960 70 0 1 {name=M50
W=min_width*switch_multiplier
L=length
nf=1
mult=32
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {ipin.sym} -1050 160 2 0 {name=p30 lab=T6
W=min_width
L=length
mult=32}
C {ipin.sym} -940 180 0 0 {name=p31 lab=T6_n
W=min_width
L=length
mult=32}
