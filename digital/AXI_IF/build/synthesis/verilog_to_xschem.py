...
Yosys: Creates inner primitive symbol + wrapper schematic + outer symbol
Verilator: Creates single primitive symbol with d_cosim
"""
...
"""
    ...
    # Instantiate inner symbol with method-specific properties
    if method == "yosys":
        # Yosys: use spice_sym_def with corrected path
        proj_name = project_name if project_name else module_name
        spice_name = spice_file if spice_file else f"{module_name}.spice"

        # Build the correct relative path from xschem/synth to runs directory
        spice_path = f"../../runs/{proj_name}_full/final/spice/{spice_name}"

        content += f"""
C {{synth/{module_name}_inner.sym}} {inst_x} {inst_y} 0 0 {{name=X1
spice_sym_def=".include {spice_path}"}}
"""
    else:  # verilator
        # Verilator: use d_cosim with .so and correct RTL path
        # Use the base module name for the .so file, not the functional variant
        ...
        content += f"""
C {{verilator/{module_name}_inner.sym}} {inst_x} {inst_y} 0 0 {{name=ADUT model={base_name}
...
"""
    ...
    # These must be positioned to match the actual pin locations on the inner symbol

    # Calculate pin positions on the inner symbol
    ...
    if len(inputs) > 0:
        ...
        for i, port in enumerate(inputs):
            ...
            # Place lab_pin on the left side of the instance, at the pin location
...
"""
    ...
    if len(outputs) > 0:
        ...
        for i, port in enumerate(outputs):
            pin_y = y_offset + spacing * (i + 1)
            # Place lab_pin on the right side of the instance, at the pin location
...
"""
    ...
    # Add input pins on left side - wire extends outward from box
    if len(inputs) > 0:
        spacing = height // (len(inputs) + 1)
        for i, port in enumerate(inputs):
            y = -height // 2 + spacing * (i + 1)
...
"""
    ...
    # Add output pins on right side - wire extends outward from box
    if len(outputs) > 0:
        spacing = height // (len(outputs) + 1)
        for i, port in enumerate(outputs):
            y = -height // 2 + spacing * (i + 1)
...
def main():
    ...
    parser.add_argument(
        "--project-name", help="Project name for SPICE path (defaults to module name)"
    )
    ...
    if args.method == "yosys":
        if not args.netlist:
            raise ValueError("--netlist required")

        # Parse netlist to get module name
        module_name, ports = parse_verilog_ports(args.netlist)

        # Use provided project_name or fall back to module_name
        project_name = args.project_name if args.project_name else module_name

        print(f"Creating Yosys symbols for: {module_name}")
        print(f"Project name: {project_name}")
        print(f"Output directory: {subdir}")

        create_inner_symbol(module_name, ports, subdir, "yosys")
        create_wrapper_schematic(
            ...
            spice_file=f"{module_name}.spice",
            project_name=project_name,
        )
        create_outer_symbol(module_name, ports, subdir)

        print(
            f"✓ Created in {subdir}/: {module_name}_inner.sym, {module_name}.sch, {module_name}.sym"
        )

    else:  # verilator
        if not args.rtl:
            raise ValueError("--rtl required")

        module_name, ports = parse_verilog_ports(args.rtl)

        # Use project_name for functional variant naming
        project_name = args.project_name if args.project_name else module_name
        ...
        # Create symbols with functional naming, but use base module_name for .so
        ...
        create_wrapper_schematic(
            ...
            subdir,
            "verilator",
            base_module_name=module_name,  # Use original module name for .so
            project_name=project_name,
        )
